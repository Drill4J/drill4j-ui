...
Lists test sessions associated with the current build. Entry point from a build to its test-run history. Rows link to the group-scoped [session page](./09-tests-results.md) with this build preselected (`?buildId=`).
...
- Row click → `/metrics/:groupId/test-sessions/:testSessionId?buildId={buildId}` (current build preselected so coverage is visible)
