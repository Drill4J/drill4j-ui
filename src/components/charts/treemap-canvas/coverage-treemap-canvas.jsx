...
export const CoverageTreemapCanvas = ({
  ...
  const [maxDepth, setMaxDepth] = useState(forceMaxDepth ?? DEFAULT_MAX_DEPTH)
  ...
  useEffect(() => {
    if (forceMaxDepth != null) {
      setMaxDepth(forceMaxDepth)
    }
  }, [forceMaxDepth])
  ...
  const renderCanvas = useCallback(() => {
    ...
    drawTreemap(ctx, positionedNodes, dpr, paintStrategy, {
      colorblindMode,
      highlightEnabled,
      highlightThreshold,
    })
  ...
  const handleContextMenu = useCallback(
    (event) => {
      ...
      onNodeContextMenu(hit.node, event)
    },
    [getHit, onNodeContextMenu]
  )
